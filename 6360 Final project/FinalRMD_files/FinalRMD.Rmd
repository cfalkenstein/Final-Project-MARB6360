...
## Research Questions: 1. Where are hotspots for YOY and juvenile bull shark abundance in Nueces and Mission-Aransas Estuaries? 2. How do environmental conditions change presence/absence of this species?
...
corpus$COMPLETION_DTTM <- dmy_hms(corpus$COMPLETION_DTTM)

# Create separate columns
corpus$YEAR  <- year(corpus$COMPLETION_DTTM)
corpus$MONTH <- month(corpus$COMPLETION_DTTM)
corpus$DAY   <- day(corpus$COMPLETION_DTTM)
corpus$TIME  <- format(corpus$COMPLETION_DTTM, "%H:%M:%S")

#Arrange dataset by year

corpus <- corpus %>% arrange(YEAR)

m <- lm(Bull_shark ~ COMPLETION_DTTM, data = Aransas)
pval <- summary(m)$coefficients[2,4]

bullcorpus <- ggplot(corpus, aes(x = COMPLETION_DTTM, y = Bull_shark)) + geom_point() + geom_smooth(method = "lm", se = FALSE) + theme_classic() + labs(title = "Shark abundance Corpus Christi Bay over time") + annotate("text", x = Inf, y = Inf, label = paste0("p = ", signif(pval, 3)), hjust = 1.5, vjust = 1)
...
nuecesmap <- leaflet(Nueces) %>%
  addTiles() %>%
  addHeatmap(
    lng = ~X,
    lat = ~Y,
    intensity = ~Bull_shark * 50,   # weights the heat by # of sharks
    blur = 1,                 # smoothing
    max = max(Nueces$Bull_shark, na.rm = TRUE),
radius = 15) %>% 
  addLegend("bottomright",
    colors = "red",
    labels = "Higher intensity = More Bull Sharks",
    title = "Bull Shark Heat Map")
...
aransasmap <- leaflet(Aransas) %>%
  ...
  addHeatmap(
    ...
    intensity = ~Bull_shark * 50,   # weights the heat by # of sharks
    blur = 1,                 # smoothing
    max = max(Aransas$Bull_shark, na.rm = TRUE),
    radius = 15) %>% 
  addLegend("bottomright",
            colors = "red",
            labels = "Higher intensity = More Bull Sharks",
...
#Copano Map 
Copano <- bullnozero %>%
  filter(MINOR_AREA_CODE == 120)

copanomap <- leaflet(Copano) %>%
  addTiles() %>%
  addHeatmap(
    lng = ~X,
    lat = ~Y,
    intensity = ~Bull_shark * 50,   # weights the heat by # of sharks
    blur = 1,                 # smoothing
    max = max(Copano$Bull_shark, na.rm = TRUE),
    radius = 15) %>% 
  addLegend("bottomright",
            colors = "red",
            labels = "Higher intensity = More Bull Sharks",
...
corpusmap <- leaflet(Corpus) %>%
  ...
  addHeatmap(
    ...
    intensity = ~Bull_shark * 50,   # weights the heat by # of sharks
    blur = 1,                 # smoothing
    max = max(Corpus$Bull_shark, na.rm = TRUE),
    radius = 15) %>% 
  addLegend("bottomright",
            colors = "red",
            labels = "Higher intensity = More Bull Sharks",
...
#Looking at Salinity
corpussalinity <- leaflet(Corpus) %>%
  addTiles() %>%
  addHeatmap(
    lng = ~X,
    lat = ~Y,
    intensity = ~START_SALINITY_NUM * 10,   # weights the heat by salinity
    blur = 1,                 
    max = max(Corpus$START_SALINITY_NUM, na.rm = TRUE),
    radius = 15) %>% 
  addLegend("bottomright",
            colors = "red",
            labels = "Higher intensity = Higher Salinity",
            title = "Salinity Heat Map")
corpussalinity
